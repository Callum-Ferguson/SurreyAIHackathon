...
COGNITIVE_SERVICE_ENDPOINT = os.environ.get("AZURE_COGNITIVE_SERVICES_MULTI_ENDPOINT")

# Cognitive service endpoint
AZURE_OPENAI_SERVICE_KEY = os.environ.get("AZURE_OPENAI_API_KEY")

# Open AI service endpoint
AZURE_OPENAI_SERVICE_ENDPOINT = os.environ.get("AZURE_OPENAI_ENDPOINT")

# Azure Open AI Deployment Model Name
AZURE_OPENAI_DEPLOYMENT_MODEL_NAME = os.environ.get("AZURE_OPENAI_DEPLOYMENT_NAME")
...
try:
    ...
    print("CallAutomationClient initialized successfully")
except Exception as e:
    print(f"Error initializing CallAutomationClient: {str(e)}")
    raise
...
async def get_chat_completions_async(user_prompt): 
    ...
    response_content = chat_handler.get_chat_response(user_prompt)
...
async def has_intent_async(user_query, intent_description, logger):
    ...
    response = chat_handler.get_chat_completions(system_prompt, combined_prompt)
